...
-- 4) Recreate helper function to update updated_at if it doesn't exist already
-- (This function is already present per project metadata, but we re-assert it for safety.)
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can view their own profile' AND schemaname = 'public' AND tablename = 'profiles'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Users can update their own profile' AND schemaname = 'public' AND tablename = 'profiles'
  ) THEN
